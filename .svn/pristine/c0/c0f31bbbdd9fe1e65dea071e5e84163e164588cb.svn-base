...
YTF
    YTF(YTF:YTFF)
"""
# _io = 'hyeri915@yantaiferry.com^연태훼리 스케쥴 297 (2020.03.16일 주).xlsx'
...
class parser():
            ...
            for j in range(0, len(excel[i])):
                ...
                for k in range(0, len(excel[i][j])):
                    ...
                    try:
                        ...
                        if "VESSEL / VOY NO." in str(excel[i][j][k]):
                            print("start ================> ", i,":",j,":",k)
    ...
    def get_routes(self, excel, i, j, k):
        ...
        for kk in range(k, len(excel[i][j])):
            ...
            if port_start_index > 0 and not self.is_merge_extend(i, j, kk) and ("" == str(excel[i][j][kk]) or None == str(excel[i][j][kk]) or "*" in str(excel[i][j][kk])):
                port_end_index = kk-1
                print("end ================> ", i,":",j,":",kk)
                break

            if port_start_index > 0 and "VESSEL / VOY NO." in str(excel[i][j][kk]):
                port_end_index = kk-1
                print("end ================> ", i,":",j,":",kk)
                break

            if port_start_index > 0 and "REMARK" in str(excel[i][j][kk]):
                port_end_index = kk-1
                print("end ================> ", i,":",j,":",kk)
                break
            ...
            if vessel_index > -1 and voy_index > -1 \
                ...
                and "VESSEL / VOY NO." not in str(excel[i][j][kk]) and "REMARK" not in str(excel[i][j][kk]) \
                and "VOY." not in str(excel[i][j][kk]) :
                ports[str(kk)] = excel[i][j][kk]
                if "\n" in ports[str(kk)]:
            ...
            if "VESSEL / VOY NO." in str(excel[i][j][kk]):
                vessel_index = kk
                voy_index = kk
                ...
                # for kk in range(len(excel[i][jj])):
                    ...
                    if kk > port_start_index -1 and kk < port_end_index + 1 :
                        ...
                        if "" != str(excel[i][jj][kk]) and "-" != str(excel[i][jj][kk]):
                            ...
                            tmp_voy = str(int(voy)).rjust(4, "0")
                            if "YANTAI" == start_port:
                                    tmp_voy = tmp_voy + "E"
                            else:
                                    tmp_voy = tmp_voy + "W"
                            vessel = "OCEAN BLUE WHALE"

                            routes.append({'line_code':self._line_code, 'vessel': vessel, 'voy': tmp_voy, 'end_route_name': end_port, 'end_route_date': end_date, 'start_route_name': start_port, 'start_route_date': start_date, 'seq':seq, 'svc':svc})
